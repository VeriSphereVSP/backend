...
from app.db import get_db, get_or_create_claim_with_embedding
...
def pgvector_topk(db: Session, claim_id: int, top_k: int):
    rows = db.execute(
        text(
            """
            WITH q AS (
...
def python_topk(db: Session, claim_id: int, query_emb, top_k: int):
    rows = db.execute(
        text(
            """
...
def compute_one(db: Session, claim_text: str, top_k: int):
    t0 = time.time()
...
def check_duplicate_batch(req: BatchCheckDuplicateRequest, db: Session = Depends(get_db)):
    try:
        return {
            "results": [
                compute_one(db, claim_text, req.top_k)
                for claim_text in req.claims
            ]
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
